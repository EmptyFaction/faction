...
use Faction\command\player\CoinFlip;
...
class Session
{
    ...
    public function setCooldown(string $key, float|int $time, array $value = []): void
    {
        ...
        $this->data["cooldowns"][$key] = array_merge([time() + $time], $value);
    }
    ...
    public function inCooldown(string $key): bool
    {
        if ($key === "combat" && $this->player->isCreative()) {
            ...
            return isset($this->data["cooldowns"][$key]) && $this->data["cooldowns"][$key][0] > time();
        }
    }

    public function getCooldownData(string $key): array
    {
        return $this->data["cooldowns"][$key] ?? [time(), null, null, null];
    }
}
