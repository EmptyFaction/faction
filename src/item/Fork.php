...
class Fork extends Durable
{
    ...
    private function changeMode(Player $player): void
    {
        ...
        if ($mode === 3) {
            ...
            $player->sendMessage(Util::PREFIX . "Votre fourche vient de passer en §cmode 5x5 §fet elle perdra donc §c3 points de durabilité §fpar utilisation !");
        } else {
            $this->changeForkMode($player, 3);
            $player->sendMessage(Util::PREFIX . "Votre fourche vient de passer en §cmode 3x3 §fet elle perdra donc §c1 point de durabilité §fpar utilisation !");
        }
    }
