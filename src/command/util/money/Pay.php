    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        if ($sender instanceof Player) {
            ...
            if (!$target instanceof Player) {
                ...
                $sender->sendMessage(Util::PREFIX . "Votre monnaie est infèrieur à §c" . floor($amount));
                return;
            }
            ...
            $sender->sendMessage(Util::PREFIX . "Vous avez envoyé un montant égal à §c" . $money . " §fà §c" . $target->getName());
            $target->sendMessage(Util::PREFIX . "Vous avez recu un montant d'argent égal à §c" . $money . " §fde la part de §c" . $sender->getName());
        }
    }
