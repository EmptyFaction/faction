...
class Market extends BaseCommand
{
        ...
        if ($sender instanceof Player) {
            ...
            $menu->setListener(InvMenu::readonly(function (DeterministicInvMenuTransaction $transaction) use ($menu, $page): void {
                ...
                if (is_null($item->getNamedTag()->getTag("id"))) {
                    ...
                    if ($item->getCustomName() === "§r§cPage Suivante") {
                        $this->addAuctionHouseItems($menu, ($page + 1));
                    } else if ($item->getCustomName() === "§r§cPage Précédente" && $page > 1) {
                        $this->addAuctionHouseItems($menu, ($page - 1));
                    } else if ($item->getCustomName() === "§r§cRéactualiser") {
                        $this->addAuctionHouseItems($menu, $page);
                    } else if ($item->getCustomName() === "§r§cMes Ventes En Cours") {
                        $this->myItems($player);
                    }
                    return;
                }
    ...
    private function addAuctionHouseItems(InvMenu $menu, int $page): void
    {
        ...
        $item = VanillaItems::DIAMOND()->setCount($page)->setCustomName("§r§cPage Actuel");
        $menu->getInventory()->setItem(45, $item);

        $item = VanillaItems::PAPER()->setCustomName("§r§cPage Précédente");
        $menu->getInventory()->setItem(48, $item);

        $item = VanillaItems::ENDER_PEARL()->setCustomName("§r§cRéactualiser");
        $menu->getInventory()->setItem(49, $item);

        $item = VanillaItems::PAPER()->setCustomName("§r§cPage Suivante");
        $menu->getInventory()->setItem(50, $item);

        $item = VanillaBlocks::CHEST()->asItem()->setCustomName("§r§cMes Ventes En Cours");
    ...
    private function checkAuctionHouse(Player $player, Item $item, int $type): void
    {
        ...
        if ($type === 0) {
            $player->sendMessage(Util::PREFIX . "Vous venez d'acheter un item à l'hotel des ventes pour §c" . $price . " §fpièces");
        } else if ($type === 1) {
