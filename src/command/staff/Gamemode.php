    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        if ($sender instanceof Player) {
            ...
            if (isset($args["joueur"])) {
                ...
                $sender->sendMessage(Util::PREFIX . "Vous venez de changer le mode de jeu de §c" . $target->getName() . " §fpour §c" . $gameModeName . " §f!");
            } else {
                $sender->setGamemode($gameMode);
                $sender->sendMessage(Util::PREFIX . "Vous venez de définir votre mode de jeu à §c" . $gameModeName . " §f!");
            }
        }
    }
