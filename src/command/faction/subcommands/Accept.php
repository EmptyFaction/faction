...
class Accept extends FactionCommand
{
    ...
    private function join(string $faction, Player $player): void
    {
        ...
        Cache::$factions[$faction]["logs"][time()] = "§c" . $player->getName() . " §fa rejoint la faction";

        $session->data["faction"] = $faction;
        $session->data["invite"] = [];

        Faction::broadcastFactionMessage($faction, "Le joueur §c" . $player->getName() . " §fvient de rejoindre votre faction");
    }
